...
log = logging.getLogger("crito.observatory")
...
class Setup(BaseModel):
    """A named optical train (OTA + camera + any reducer/barlow) for the exposure
    planner. Selecting one auto-fills focal length + pixel size; ``calibration_file``
    optionally points it at its own measured sensor table. Purely descriptive — it
    does not bind any device."""
    id: str = "setup"
    name: str | None = None
    focal_length_mm: float | None = None
    pixel_size_um: float | None = None
    camera: str | None = None            # which camera this train uses (informational)
    calibration_file: str | None = None  # per-setup calibration table (optional)


class Observatory(BaseModel):
    id: str = "crito"
    name: str = "CRITO Observatory"
    observer: str = "CRITO"
    ...
    setups: list[Setup] = []         # optical trains for the exposure planner

    def camera(self, role: str = "camera") -> CameraSpec | None:
        return next((c for c in self.equipment.cameras if c.role == role), None)

    def setups_view(self) -> list[dict]:
        """Exposure-planner setups. If none are declared, synthesize one from the
        configured OTA + science camera so the planner always has a default."""
        def view(s: Setup) -> dict:
            return {"id": s.id, "name": s.name or s.id,
                    "focal_length_mm": s.focal_length_mm, "pixel_size_um": s.pixel_size_um,
                    "camera": s.camera, "calibration_file": s.calibration_file}
        if self.setups:
            return [view(s) for s in self.setups]
        cam = self.camera("camera")
        tel = self.equipment.telescope
        name = " + ".join(x for x in [tel.name, cam.name if cam else None] if x) or "default"
        return [{"id": "default", "name": name,
                 "focal_length_mm": tel.focal_length_mm,
                 "pixel_size_um": cam.pixel_size_um if cam else None,
                 "camera": cam.name if cam else None, "calibration_file": None}]
