...
(``CRITO_WEATHER_DEVICE``). No data + enforcement on = UNSAFE, which correctly
...
log = logging.getLogger("crito.agent.safety")
    ...
    async def _on_transition(self, old: str, new: str, reasons: list[str]) -> None:
        ...
        if protective and not self.override:
            ...
            if notifier is not None:
                try:
                    await notifier.email.send("[CRITO] SAFETY " + new.upper(), msg)
                except Exception:
                    pass
        elif act:
