...
mirrors all of them since state lives in the DB):
  ...
  • Slack (primary): a CRITO-owned ``slack_sdk`` Socket Mode bot posts an
...
log = logging.getLogger("crito.transient.approvals")
...
class ApprovalService:
    ...
    async def _email_post(self, c: dict) -> None:
        ...
        await self.email.send(f"[CRITO] candidate {oid} ({c.get('class_label') or '?'})",
                              "\n".join(lines), html)
